...
#include "stm32g4xx_hal.h" // Include HAL header for GPIO definitions
 ...
 * indicators (Green=RUN/TIM4, Red=ERR/TIM2, driven via the LED_xxx_ON/OFF
 * macros below) and the application LED module (led_control.c, which owns the
 * Blue channel and is controlled over OD 0x2000).
 *
 * When the application takes manual control (OD 0x2000 != Normal) it asserts
 * this flag via MCOTGT_SetLEDOverride(1); the stack's macro writes then become
 * no-ops so the stack's Red/Green cannot co-light with / mask the application's
 * Blue indication on the shared common-anode emitter. On release (flag = 0) the
 * next stack LED tick repaints RUN/ERR automatically.
 *
