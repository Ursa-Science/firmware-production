...
// Bit time settings for 80 MHz FDCAN clock
// Sample point ~87.5% for all bitrates
// 16 TQ per bit: 1 sync + (SEG1+1) prop/phase1 + (SEG2+1) phase2 = 1+13+2 = 16
// The macros are register values, actual values are register values+1
...
// 800k needs TQ=25 (1+21+3) instead of TQ=16 (1+13+2)
#define CANA_SEG1(portnum,bitratekpbs) ((bitratekpbs== 800) ? 20 : 12)
#define CANA_SEG2(portnum,bitratekpbs) ((bitratekpbs== 800) ?  2 :  1)
#define CANA_SJW(portnum,bitratekpbs)   ((bitratekpbs== 800) ?  2 :  1)
  ...
  if (i<NUM_CAN_PORTS)
  {
    ...
    // INSERT APPLICATION SPECIFIC CODE AS NEEDED
      ...
      if (ErrorStatusITs & FDCAN_IT_BUS_OFF)  /* Bus_Off Status  */
      {
        ...
        CONFIG_USED.LEDErr = LED_ON;
#endif
				// Bus-off recovery: re-initialize FDCAN to exit bus-off state
				// ISO 11898-1: controller must see 128x11 recessive bits before rejoining
				// HAL_FDCAN_Stop + HAL_FDCAN_Start resets the protocol engine
...
// CAN CONFIGURATION
  ...
  gCANHandle[CAN_PORT_INDEX].Init.TransmitPause = DISABLE;
	gCANHandle[CAN_PORT_INDEX].Init.ProtocolException = DISABLE;
