...
// RUN/ERR LEDs (Green/Red, default); non-0 = the application (led_control.c,
// OD 0x2000) owns the LEDs and the stack's LED writes are suppressed.
...
uint16_t timeout;
	...
	//Fire event for fatal error, allowing other modules to react (e.g. log error, update status display, etc.)
...
uint8_t MCOUSER_ResetCommunication (void)
{
  ...
  result = MCO_DefaultResetCommunication(NODEID,CAN_BITRATE,CAN_BRS_BITRATE,DEFAULT_HEARTBEAT);
	...
	/* Re-assert TPDO2 mapping (Error Register -> 0x288).
	 *
	 * MCO_DefaultResetCommunication() runs MCO_UpdateSystemFromOD(), which rebuilds
	 * every PDO length from the OD mapping table. For TPDO2 it processes 0x1A01 and
	 * looks up the *first* mapped object (Error Register, 0x1001) in the process-image
	 * OD table to set the PDO base offset. 0x1001 is not in that table (CANopen
	 * Architect special-cases the Error Register to ODLOC_UNKNOWN regardless of
	 * Storage/Access), so the lookup fails and TPDO2 length is left 0 -> the valve
	 * transmits an empty DLC=0 frame. (The pump avoids this only because its TPDO2
	 * leads with PumpState, a proc-image object, so 0x1001 rides along as a later
	 * contiguous byte and is never looked up.)
	 *
	 * MCO_InitTPDO() sets CANmsg.LEN and the offset directly, with no proc-table
	 * lookup, so re-asserting here restores length 1 / offset P100100_Error_Register
