...
/**************************************************************************
Version Information, here V7.17
**************************************************************************/
#define _MCOPVERSION_ 7
#define _MCOSUBPVERSION_ 17
...
#define ERROFL_EMCY  0x4810 // Transmit buffer overflow, EMCY message lost
#define ERROFL_PDO   0x4820 // Transmit buffer overflow, TPDO message lost
#define ERROFL_SDO   0x4830 // Transmit buffer overflow, (U)SDO message lost
#define ERROFL_HBT   0x4840 // Transmit buffer overflow, Heartbeat message lost
#define ERROFL_PMP   0x6000 // XPDO_UpdatePDOMapping processing error base
...
#define ERRFT_NVMEM  0x8212 // NVOL storage size too small
#define ERRFT_HBIP   0x9901 // MCOP_InitHBConsumer parameters out of range
#define ERRFT_HBIFLT 0x9902 // MCOP_InitHBConsumer out of CAN receive filters
#define ERRFT_HBCFLT 0x9903 // MCOP_ProcessHBCheck parameters out of range
#define ERRFT_BERR   0xF6F6 // CAN Bus Error
...
**************************************************************************/
#define GEN_RD32(level,ptr) MCO_GenRd32((level),(ptr))

#define GEN_RD24(level,ptr) MCO_GenRd24((level),(ptr))

#define GEN_RD16(level,ptr) MCO_GenRd16((level),(ptr))

#define GEN_RD8(level,ptr)  MCO_GenRd8((level),(ptr))

#define GEN_WR32(level,ptr,value) MCO_GenWr32((level),(ptr),(value))

#define GEN_WR24(level,ptr,value) MCO_GenWr24((level),(ptr),(value))

#define GEN_WR16(level,ptr,value) MCO_GenWr16((level),(ptr),(value))

#define GEN_WR8(level,ptr,value)  MCO_GenWr8((level),(ptr),(value))
...
#if USE_EXTENDED_SDO
 #if USE_GENOD_PTR
  #define ODGENTRYC(index,sub,acc,len,ptr) { GETBYTES16(index), sub, acc, GETBYTES16(len), (uint8_t *) ptr }
...
// macro to return the offset of a pointer relative to the start
// of the PI (can be negative)
#define OFFSET_PI(chr_ptr) ((long int)(chr_ptr)-(long int)&gProcImg[0])

// macro returns TRUE if pointer is inside the PI
#define INSIDE_PI(chr_ptr) ( (OFFSET_PI(chr_ptr) >= 0) && \
                             (OFFSET_PI(chr_ptr) < PROCIMG_SIZE) )
...
#endif
// Macro to find out actual message length for given payload size
#define LAYER_MSG_LENGTH(mslen) ( \
  (mslen <=  8) ? mslen : \
  (mslen <= 12) ? 12 : \
  (mslen <= 16) ? 16 : \
  (mslen <= 20) ? 20 : \
...
// CAN-IDs for LSS 
#define LSS_MANAGER_ID   0x7E5UL
#define LSS_SERVER_ID    0x7E4UL
#define LSS_CFGREQ_ID    0x7E6UL
#define LSS_FBMIN_ID     0x7D0UL
#define LSS_FBMAX_ID     0x7EFUL
#define LSS_FB29_ID_MSK  0x1FF80000UL
#define LSS_FB29_ID_CMP  0x1F900000UL


/**************************************************************************
MACROS FOR CAN-ID USAGE
**************************************************************************/
#define IS_CANID_LSS_REQUEST(canid) ((canid) == LSS_MANAGER_ID)

#if USE_29BIT_LSSFEEDBACK
  #define IS_CANID_LSS_RESPONSE(canid) ( ((canid) == LSS_SERVER_ID) || ((canid) == LSS_CFGREQ_ID) || (((canid) >= LSS_FBMIN_ID) && ((canid) < LSS_FBMAX_ID)) || ((canid & LSS_FB29_ID_MSK) == LSS_FB29_ID_CMP) )
#else
  #define IS_CANID_LSS_RESPONSE(canid) ( ((canid) == LSS_SERVER_ID) || ((canid) == LSS_CFGREQ_ID) || (((canid) >= LSS_FBMIN_ID) && ((canid) < LSS_FBMAX_ID)) )
#endif
...
#define IS_CAN_ID_MY_HB(canid) ((canid) == 0x700 + MY_NODE_ID)
  ...
  #define IS_CANID_RESTRICTED(canid) ( ((canid) < 0x0080) \
    ...
    || (((canid) >= 0x01C0) && ((canid) <  0x0200)) \
    || (((canid) >= 0x0240) && ((canid) <  0x0280)) \
    || (((canid) >= 0x02D0) && ((canid) <  0x0300)) \
    || (((canid) >= 0x0340) && ((canid) <  0x0380)) \
    || (((canid) >= 0x02C0) && ((canid) <  0x0300)) \
    || (((canid) >= 0x0440) && ((canid) <  0x0480)) \
    || (((canid) >= 0x04C0) && ((canid) <  0x0500)) \
    || (((canid) >= 0x0540) && ((canid) <  0x0580)) \
  ...
  #define IS_CAN_ID_SDOREQUEST(canid) ( \
      ...
      || (    ((canid) > 0x0240) \
           && ((canid) < 0x0580) \
           && (((canid) & 0x00CF) == (0x0040 + MY_NODE_ID - 1)) \
         ) \
    )

  #define IS_CAN_ID_ANY_SDOREQUEST(canid) (IS_CAN_ID_SDOREQUEST(canid))

  #define IS_CAN_ID_SDOREQUEST_TX(canid) ( \
              ((canid) >= 0x0240) \
           && ((canid) <  0x0580) \
           && (((canid) & 0x0730) \
               == (((MY_NODE_ID - 1) & 0x0C) << 6) + (((MY_NODE_ID - 1) & 0x03) << 4) \
             ) \
    )
  ...
  #define CAN_ID_PDO_CiA447(node) (0x40000180ul + (((uint16_t)((node-1)&7)<<7)) + (((uint16_t)((node-1)&8))<<2))

  #define IS_NODE_CiA301_CLIENT() (MY_NODE_ID == 1)

  #define IS_CAN_ID_MINE(canid) (((canid) == 0x080 + MY_NODE_ID) || ((canid) == 0x580 + MY_NODE_ID) \
            || (IS_CAN_ID_SDOREQUEST_TX((canid))) ||  (IS_CAN_ID_SDORESPONSE_TX((canid))) \
            || ((canid) == CAN_ID_PDO_CiA447(MY_NODE_ID)) \
    )
  ...
  #if defined(USE_CANOPEN_FD) && (USE_CANOPEN_FD==1)
    #define IS_CAN_ID_USDOREQUEST(canid) (((canid) >= 0x601) && ((canid) <= 0x67F))
    #define IS_CAN_ID_USDORESPONSE(canid) (((canid) >= 0x581) && ((canid) <= 0x5FF))
    #define CAN_ID_USDORESPONSE_FROM_RXID(canid) (0x580+MY_NODE_ID)
    #define CAN_ID_USDOREQUEST(client_nodeid, server_nodeid)  ( 0x600 + client_nodeid )
    #define CAN_ID_USDORESPONSE(client_nodeid, server_nodeid) ( 0x580 + server_nodeid )
    #define USDOSERVER(tx_canid) 0
  #else
    #define IS_CAN_ID_SDOREQUEST(canid) ((canid) == 0x600+MY_NODE_ID)
    #define IS_CAN_ID_ANY_SDOREQUEST(canid) (((canid) >= 0x601) && ((canid) <= 0x67F))
...
/**************************************************************************
Utility macros to calculate minimum or maximum values.
**************************************************************************/
#define MINVAL(vala,valb) (((vala)<(valb)) ? (vala) : (valb))
#define MAXVAL(vala,valb) (((vala)>(valb)) ? (vala) : (valb))


/**************************************************************************
Default log levels for debug output.
**************************************************************************/
#define MCO_LOG_MIN     1
#define MCO_LOG_ERROR   1
#define MCO_LOG_WARNING 2
#define MCO_LOG_INFO    3
#define MCO_LOG_DEBUG   4
#define MCO_LOG_MAX     4
...
typedef struct
{
  ...
  uint16_t Bitrate;            // Current bitrate in kbit
#if defined(USE_CANOPEN_FD) && (USE_CANOPEN_FD==1)
  uint16_t BRSBitrate;         // For CAN FD: Data (BRS) bitrate in kbit
#endif
  ...
  uint8_t saved_flags;         // A set bit indicates that saved parameters are used
...
typedef uint8_t ODLocationType;
#define ODLOC_UNKNOWN  ((ODLocationType)0)
#define ODLOC_SDOREPLY ((ODLocationType)1)
#define ODLOC_ODENTRY  ((ODLocationType)2)
#define ODLOC_ODGENTRY ((ODLocationType)3)
...
// Extended OD tables
extern uint32_t MEM_CONST gODIDList[];
extern uint8_t MEM_CONST gODIDType[];
extern ODLocationType MEM_CONST gODIDLocation[];
extern int MEM_CONST gSizeofgODIDList;
#endif
...
uint8_t MCO_DefaultResetCommunication (
  uint8_t nodeID,       // CANopen node ID (1-126 or 0 if not yet set)
...
RETURNS: FALSE, if initialization was not successful
         TRUE, if initialization was successful
**************************************************************************/
uint8_t MCOUSER_ResetCommunication (
  void
  );
...
void MCOUSER_FatalError (
  uint16_t ErrCode // To debug, search source code for the ErrCode encountered
  );
...
*******************************************************************************/
uint8_t MCOUSER_XSDOWriteSegment (
  uint8_t last, // Is set to 1 if this is the last segment
...
*******************************************************************************/
uint8_t MCOUSER_UnknownReceived(
  CAN_MSG *pRxCAN // Pointer to CAN message received
  );
#endif
...
***************************************************************************/
uint8_t XPDO_ResetPDOMapEntry (
  uint8_t TxRx, // Set to 0 for TPDO, 1 for RPDO
...
/*******************************************************************************
DOES:    Call Back function to keep the stack processing going for blocking
         SDO/USDO clients functions.
NOTE:    Customize if your application needs processing other functions at all
         times.
RETURNS: Nothing
*******************************************************************************/
void MCOUSER_ProcessStack(
  void
);
...
void MCOUSER_NodeSpecificSDOWrite (
  uint8_t node, // node id of node that made the request (if known, else zero)
  uint16_t idx,
  uint8_t sub,
  uint16_t offset,
  uint8_t len
